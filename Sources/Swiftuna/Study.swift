...
public final class Study: @unchecked Sendable {
    ...
    internal init(raw: OpaquePointer?, name: String, direction: Direction) {
        self.raw = raw
        self.name = name
    ...
    public func setUserAttr(
        ...
        if status != 0 {
            throw SwiftunaError.fromLastError(fallbackCode: status, context: "Failed to set study user attribute '\(key)'")
        }
    }
    ...
    public func userAttr(_ key: String) throws(SwiftunaError) -> String? {
        ...
        if status != 0 {
            throw SwiftunaError.fromLastError(fallbackCode: status, context: "Failed to get study user attribute '\(key)'")
        }
    ...
    public func enqueue(
        ...
        if !userAttrs.isEmpty {
            guard let attrsData = try? JSONEncoder().encode(userAttrs),
                  let str = String(data: attrsData, encoding: .utf8) else {
