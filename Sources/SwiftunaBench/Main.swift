...
func computeStats(_ samples: [Double]) -> Stats {
    ...
    return Stats(
        median: median, mean: mean, stdev: stdev, min: sorted.first!, max: sorted.last!,
        cvPct: mean > 0 ? stdev / mean * 100 : 0, samples: sorted)
}
...
func benchPaired(nTrials: Int, iters: Int, verbose: Bool, swiftBody: () throws -> Double, rustBody: () -> Double?)
    throws -> (swift: Stats, rust: Stats, paired: Stats)
{
    var sw: [Double] = []
    var ru: [Double] = []
    var pd: [Double] = []
    sw.reserveCapacity(iters)
    ru.reserveCapacity(iters)
    pd.reserveCapacity(iters)
    for i in 0..<iters {
        let label = String(format: " %2d/%2d", i + 1, iters)
        if !verbose {
            // inline progress: \r
            let bar =
                String(repeating: "█", count: (i * 20) / iters) + String(repeating: "░", count: 20 - (i * 20) / iters)
...
func swiftE2E(nTrials: Int) throws {
    let study = try Swiftuna.createStudy(
        name: "bench_e2e_\(UUID().uuidString)", direction: .minimize, sampler: TPESampler(seed: 42))
    for _ in 0..<nTrials {
        var t = try study.ask()
...
func swiftRandomNoTPE(nTrials: Int) throws {
    let study = try Swiftuna.createStudy(
        name: "bench_rand_\(UUID().uuidString)", direction: .minimize, sampler: RandomSampler(seed: 42))
    for _ in 0..<nTrials {
        var t = try study.ask()
        let x = try t.suggest("x", in: -10.0...10.0)
        let y = try t.suggest("y", in: -10.0...10.0)
...
func verdict(pct: Double, cv: Double, stdev: Double) -> String {
    if abs(pct) < 5, cv < 5, stdev < 5 {
        return "\(ANSI.green)✓ zero overhead\(ANSI.reset) \(ANSI.dim)(<5%, CV<5%, paired σ<5µs)\(ANSI.reset)"
    }
    if stdev > 10 {
        return "\(ANSI.yellow)⚠ high jitter\(ANSI.reset) \(ANSI.dim)(close apps, plug in power)\(ANSI.reset)"
    }
    ...
    static func main() {
        ...
        do {
            ...
            if !cfg.json {
                ...
                print(
                    dim("│ n=\(cfg.nTrials)  iters=\(cfg.iters)  warmup=\(cfg.warmupIters)  release  ")
                        + c("●", ANSI.green))
                print(dim("└──────────────────────────────────────────────────────"))
                fflush(stdout)
            }
            ...
            // Warmup (not measured)
            if !cfg.json {
                print("\n\(c("warmup", ANSI.cyan)) \(cfg.warmupIters)×\(cfg.nTrials / 2) …", terminator: "")
                fflush(stdout)
            }
            ...
            if !cfg.json {
                print(
                    "\n\(c("● E2E TPE", ANSI.bold))  \(ANSI.dim)quadratic x∈[-10,10] y∈[-10,10]  TPE(seed=42)\(ANSI.reset)"
                )
                fflush(stdout)
            }
            let (swiftRaw, rustRaw, pairedRaw) = try benchPaired(
                nTrials: cfg.nTrials, iters: cfg.iters, verbose: cfg.verbose,
            ...
            if !cfg.json {
                ...
                func row(_ name: String, _ s: Stats) {
                    print(
                        String(
                            format: "  │ %-12@ │ %6.1f │ %6.1f │ %5.2f │ %4.1f%% │ %6.1f │ %6.1f │",
                ...
                print(
                    "  paired median \(fmt(pairedStats.median, digits: 2))  mean \(fmt(pairedStats.mean, digits: 2)) ± \(fmt(ci, digits: 2)) (95% CI)  σ \(fmt(pairedStats.stdev, digits: 2))"
                )
                print(
                    "  median diff \(fmt(swiftStats.median - rustStats.median, digits: 2)) µs  Swift \(fmt(swiftStats.median)) vs Rust \(fmt(rustStats.median))"
                )
                print(
                    "  \(verdict(pct: pctPaired, cv: max(swiftStats.cvPct, rustStats.cvPct), stdev: pairedStats.stdev))"
                )
            }

            // Random FFI
            if !cfg.json {
                print("\n\(c("● Random", ANSI.bold))  \(ANSI.dim)isolates FFI (no TPE)\(ANSI.reset)")
                fflush(stdout)
            }
            let (swRandRaw, ruRandRaw, paRandRaw) = try benchPaired(
                nTrials: cfg.nTrials, iters: cfg.iters, verbose: cfg.verbose,
            ...
            if !cfg.json {
                ...
                func r2(_ n: String, _ s: Stats) {
                    print(
                        String(
                            format: "  │ %-12@ │ %6.2f │ %6.2f │ %5.3f │ %4.1f%% │", n as NSString, s.median, s.mean,
                            s.stdev, s.cvPct))
                }
                ...
                print(dim("  └──────────────┴────────┴────────┴───────┴──────┘"))
                print(
                    "  FFI Δ \(fmt(paRand.median, digits: 3)) µs (\(String(format: "%+.1f%%", randPct)))  \(String(format: "%.3f×", randFactor))  σ \(fmt(paRand.stdev, digits: 3))"
                )
            }
            ...
            if !cfg.json {
                print(
                    "\n\(c("● Trials", ANSI.bold))  \(ANSI.dim)deserialization  100 rich trials ×1000 fetches\(ANSI.reset)"
                )
                fflush(stdout)
            }
            ...
            let qMs = (Double(qElapsed.components.seconds) + Double(qElapsed.components.attoseconds) * 1e-18) * 1_000
            let qUs = qMs * 1_000 / Double(qTotal)
            if !cfg.json {
                print(
                    String(
                        format: "  %.1f ms total  %.2f µs/trial  %.0f trials/s  (%d trials)", qMs, qUs,
                        Double(qTotal) / (qMs / 1_000), qTotal))
                print(
            ...
            if cfg.json {
                let out: [String: Any] = [
                    ...
                    "swift": [
                        "median": swiftStats.median, "mean": swiftStats.mean, "stdev": swiftStats.stdev,
                        "cv": swiftStats.cvPct,
                    ],
                    "rust": [
                        "median": rustStats.median, "mean": rustStats.mean, "stdev": rustStats.stdev,
                        "cv": rustStats.cvPct,
                    ],
                    "paired": [
                        "median": pairedStats.median, "mean": pairedStats.mean, "stdev": pairedStats.stdev,
                        "pct": pctPaired, "factor": pairedFactor, "ci95": ci,
                    ],
                    "random": [
                        "swiftMedian": swRand.median, "rustMedian": ruRand.median, "pairedMedian": paRand.median,
                        "factor": randFactor,
                    ],
                    "trials": ["usPerTrial": qUs, "throughput": Double(qTotal) / (qMs / 1_000)],
                ]
